...
[NHK->語学](https://www.nhk.or.jp/gogaku/#indexMainList) https://www.nhk.or.jp/gogaku/#indexMainList  

- ロシア語限定版には [nhkradio-russian-record](https://github.com/ru-museum/nhkradio-russian-record) があります。
...
- シェルは **bash** を指定して下さい。
### （0）環境構築   
- 作業フォルダに nhkradio-langs-record.sh を置きアクセス権限を変更します。  
```
...
```
# 「まいにちロシア語」（ID-14）の設定例：　
30 2 * * 1-5 username bash /your/directory/nhkradio-langs-record.sh -i 14
　　⇨ /まいにちロシア語/まいにちロシア語:初級編-20260331(火)02:30.m4a として保存されます。

# 保存ディレクトリの指定：　
30 2 * * 1-5 username bash /your/directory/nhkradio-langs-record.sh -i 14 -d nhk
...
50 7 * * 1-5 sleep 50; username bash /your/directory/nhkradio-langs-record.sh …………  
```
...
# CRON
49 8 * * 1-5 sleep 40; username bash /your/directory/nhkradio-langs-record.sh ………… 
```
### (４) 番組表以外の録音：   
- 番組表以外の番組(AM/FM)の録音が出来ます。
```
# 30分後AM放送を「NHKニュース」として10分間録音し「audio」ディレクトリに保存：　
$ bash ./nhkradio-langs-record.sh -w am -r 00:10:00 -t NHKニュース -d audio -s 30m
...
- 録音時間を**10秒**程度に設定し実動前のテストを行って下さい。  
 　※ シェルは **bash** を使用して下さい。
```
$ bash ./nhkradio-langs-record.sh -i 14 -r 00:00:10
...
- データは編集可能となっています。   
- **ロシア語限定版**はこちらにあります。[nhkradio-russian-record](https://github.com/ru-museum/nhkradio-russian-record)
...
　　[GitHub Pages: nhkradio-langs-record](https://ru-museum.github.io/nhkradio-langs-record/)  
　　[https://ru-museum.github.io/nhkradio-langs-record/](https://ru-museum.github.io/nhkradio-langs-record/)  
-->
